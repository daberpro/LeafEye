...
namespace winrt::LeafEye::implementation
{
	...
	void HistoryPage::OnNavigatedTo(winrt::Microsoft::UI::Xaml::Navigation::NavigationEventArgs const& e) {
		...
		RegisterPaginationCallback(
			...
			[this, database](int64_t offset, int64_t limit) -> winrt::fire_and_forget {
				auto start_time = m_startTime;
				auto end_time = m_endTime;
				winrt::apartment_context ui_thread;
				co_await winrt::resume_background();
				auto result = co_await database.GetHistoryByDateRange(start_time, end_time, offset, limit);
				co_await ui_thread;
				if (!result.IsError() && result.IsValueExists()) {
					auto history = result.ResultValue().as<
						winrt::Windows::Foundation::Collections::IVector<winrt::LeafEyeCore::HistoryModel>
	...
	void HistoryPage::StartDate_DateChanged(winrt::Microsoft::UI::Xaml::Controls::CalendarDatePicker const& sender, winrt::Microsoft::UI::Xaml::Controls::CalendarDatePickerDateChangedEventArgs const& args)
	{
		if (auto newDate = args.NewDate())
		{
			winrt::Windows::Foundation::DateTime winrtDate = newDate.Value();
			auto sysTime = winrt::clock::to_sys(winrtDate);
			uint64_t milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(sysTime.time_since_epoch()).count();
			if (milliseconds != m_startTime) {
				m_startTime = milliseconds;
			}

			SetPaginationFilter(HistoryFileType::DATE);
		}
	}

	void HistoryPage::EndDate_DateChanged(winrt::Microsoft::UI::Xaml::Controls::CalendarDatePicker const& sender, winrt::Microsoft::UI::Xaml::Controls::CalendarDatePickerDateChangedEventArgs const& args)
	{
		if (auto newDate = args.NewDate())
		{
			winrt::Windows::Foundation::DateTime winrtDate = newDate.Value();
	...
	winrt::fire_and_forget HistoryPage::FileHistoryListView_ItemClick(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::Controls::ItemClickEventArgs const& e)
	{
		auto model = e.ClickedItem().as<LeafEyeCore::FileHistoryModel>();
		auto dialog = LeafEye::FileHistoryDetailDialog(model);
